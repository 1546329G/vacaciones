...
const String _baseUrl = 'https://sector7.site';
...
class ApiService {
  ...
  Future<User> registerUser(String email, String password, String rol) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        // 201 Created es una respuesta estándar para registro exitoso
        ...
        final User newUser = User.fromJson(
          responseData['user'] ?? {},
        ); // Asegúrate de que 'user' no sea nulo en la respuesta
        ...
        print(
          'Error de registro (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al registrar usuario: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
  ...
  Future<User> loginUser(String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // 200 OK es una respuesta estándar para login exitoso
        final responseData = json.decode(response.body);
        ...
        if (token != null && token.isNotEmpty) {
          ...
          print(
            'Advertencia: El API de login no retornó un token JWT. Asegúrate de añadirlo en el backend.',
          );
          // Si tu backend no devuelve un token, la sesión no se mantendrá persistente automáticamente.
        }
        print(
          'Inicio de sesión exitoso. Usuario: ${user.email}, Rol: ${user.rol}',
        );
        return user;
      } else {
        final errorData = json.decode(response.body);
        print(
          'Error de login (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al iniciar sesión: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
      print('Excepción durante el login: $e');
      throw Exception(
        'Error de conexión o inesperado durante el inicio de sesión: $e',
      );
    }
  }
  ...
  Map<String, String> _getAuthHeaders() {
    if (_authToken == null) {
      ...
      throw Exception(
        'No hay token de autenticación disponible. Inicia sesión primero.',
      );
    }
    return {
      'Content-Type': 'application/json',
      'Authorization':
          'Bearer $_authToken', // Envía el token en el header Authorization
    };
  }
  ...
  Future<User> getOwnProfile() async {
    final url = Uri.parse(
      '$_baseUrl/users/me',
    ); // Ejemplo de endpoint para perfil
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        print(
          'Error al obtener perfil (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al obtener perfil: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
  ...
  Future<User> updateOwnProfile(String? nombre, String? telefono) async {
    final url = Uri.parse(
      '$_baseUrl/users/me',
    ); // Ejemplo de endpoint para actualizar perfil
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        print(
          'Error al actualizar perfil (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al actualizar perfil: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
      print('Excepción al actualizar perfil: $e');
      throw Exception(
        'Error de conexión o inesperado al actualizar perfil: $e',
      );
    }
  }
  ...
  Future<Address> addAddress(Address address) async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode(
          address.toJson(),
        ), // Convierte el objeto Address a JSON
      );

      if (response.statusCode == 201) {
        // 201 Created es común para añadir recursos
        final responseData = json.decode(response.body);
        return Address.fromJson(responseData['address']);
      } else {
        final errorData = json.decode(response.body);
        print(
          'Error al añadir dirección (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al añadir dirección: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> addressListJson =
            responseData['data'] ??
            []; // Asegura que 'data' no sea nulo y sea una lista
        return addressListJson.map((json) => Address.fromJson(json)).toList();
      } else {
        final errorData = json.decode(response.body);
        print(
          'Error al obtener direcciones (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al obtener direcciones: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
      print('Excepción al obtener direcciones: $e');
      throw Exception(
        'Error de conexión o inesperado al obtener direcciones: $e',
      );
    }
  }
  ...
  Future<Address> updateAddress(int addressId, Address address) async {
    final url = Uri.parse(
      '$_baseUrl/users/me/addresses/$addressId',
    ); // Endpoint para actualizar dirección por ID
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        print(
          'Error al actualizar dirección (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al actualizar dirección: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
      print('Excepción al actualizar dirección: $e');
      throw Exception(
        'Error de conexión o inesperado al actualizar dirección: $e',
      );
    }
  }
  ...
  Future<void> deleteAddress(int addressId) async {
    final url = Uri.parse(
      '$_baseUrl/users/me/addresses/$addressId',
    ); // Endpoint para eliminar dirección por ID
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        print(
          'Error al eliminar dirección (Status ${response.statusCode}): ${errorData['error'] ?? response.reasonPhrase}',
        );
        throw Exception(
          'Error al eliminar dirección: ${errorData['error'] ?? response.reasonPhrase}',
        );
      }
    } catch (e) {
      print('Excepción al eliminar dirección: $e');
      throw Exception(
        'Error de conexión o inesperado al eliminar dirección: $e',
      );
    }
  }
}
